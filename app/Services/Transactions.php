...
class Transactions
{
    ...
    {
        $request["transaction_desc"] = trans(
            "api.transaction.admin_commission"
        );
    ...
    protected function createFleetWallet($request)
    {
        ...
        if (empty($fleet->wallet_balance)) {
            ...
            $fleetWallet->close_balance =
                $fleet->wallet_balance + $request["amount"];
        }
    ...
    public function providerDiscountCredit($request)
    {
        ...
        $request["transaction_desc"] = trans(
            "api.transaction.discount_recharge"
        );
    ...
    public function waitingAmount($request)
    {
        ...
        $ad_det_amt = -1 * abs($request["amount"]);
        $request["transaction_desc"] = trans(
            "api.transaction.waiting_commission"
        );
        $request["id"] = $request["id"];
        $request["type"] = "D";
        $request["amount"] = $ad_det_amt;
    ...
    public function providerRideCredit($request)
    {
        ...
        if ($admin_amount > 0) {
            $request["transaction_desc"] = trans(
                "api.transaction.provider_recharge"
            );
    ...
    public function AdminAddAmountCreditDebit($request, $type = 1)
    {
        ...
        $transaction_alias =
            "AP" . str_pad($transaction_id, 6, 0, STR_PAD_LEFT);
    ...
    public function transationAlias($userType, $paymentType = null)
    {
        ...
        $respone["transaction_alias"] =
            $prefix . str_pad($transaction_id, 6, 0, STR_PAD_LEFT);
    ...
    public function disputeCreditDebit($request, $type = 1)
    {
        if ($type == 1) {
            ...
            $transaction_alias =
                "DPU" . str_pad($transaction_id, 6, 0, STR_PAD_LEFT);
            ...
            $transaction_alias =
                "DPP" . str_pad($transaction_id, 6, 0, STR_PAD_LEFT);
