...
class Ride
{
    ...
    {
        $geofence = $this->poly_check_request(
            round($request->s_latitude, 6),
            round($request->s_longitude, 6)
        );

        if ($geofence == false) {
            return [
                "status" => 400,
                "message" => trans("user.ride.service_not_available_location"),
                "error" => trans("user.ride.service_not_available_location"),
            ];
        }

        $ride_city_price = RideCityPrice::where("geofence_id", $geofence)
        ...
        $timezone = Auth::guard("user")->user()->state_id
            ? State::find($this->user->state_id)->timezone
            : "";
        ...
        $distance = $this->settings->transport->provider_search_radius
            ? $this->settings->transport->provider_search_radius
            : 100;
        ...
        $wheel_chair =
            $request->wheel_chair != null ? $request->wheel_chair : 0;
        ...
        $request->request->add([
            "provider_negative_balance" =>
                $this->settings->site->provider_negative_balance,
        ]);
        ...
        $whereHasCallback = [
            "service" => $callback,
            "service.vehicle" => $childseat,
        ];

        $Providers = (new UserServices())->availableProviders(
            $request,
            $withCallback,
            $whereHasCallback
        );
        ...
        if (count($Providers) == 0) {
            return [
                "status" => 422,
                "message" => trans("api.ride.no_providers_found"),
            ];
        }
        ...
        try {
            ...
            $rideRequest->distance =
                count($details["routes"]) > 0
                    ? $details["routes"][0]["legs"][0]["distance"]["value"] /
                    1000
                    : 0;
            ...
            if ($request->has("someone") && $request->someone == 1) {
                ...
                try {
                    if (
                        !empty($this->settings->site->send_email) &&
                        $this->settings->site->send_email == 1
                    ) {
                        Mail::send(
                            "mails/someone",
                            [
                            ...
                            function ($mail) use ($request) {
                                $mail->from(
                                    $this->settings->site->mail_from_address,
                                    $this->settings->site->mail_from_name
                                );
                                $mail
                                    ->to(
                                        $request->someone_email,
            ...
            $rideRequest->destination_log = json_encode([
                [
                    "latitude" => $rideRequest->d_latitude,
                    "longitude" => $request->d_longitude,
                    "address" => $request->d_address,
                ],
            ]);
            $rideRequest->unit = isset($this->settings->site->distance)
                ? $this->settings->site->distance
                : "Kms";
            ...
            if (
                $Providers->count() <=
                (isset($this->settings->transport->surge_trigger)
                    ? $this->settings->transport->surge_trigger
                    : 0) &&
                $Providers->count() > 0
            ) {
                $rideRequest->surge = 1;
            ...
            if (
                ...
                $rideRequest->schedule_at = Carbon::createFromFormat(
                    "Y-m-d H:i:s",
                    Carbon::parse(
                        $request->schedule_date . " " . $request->schedule_time
                    )->format("Y-m-d H:i:s"),
            ...
            $rideRequest = RideRequest::with("ride", "ride_type")
                ->where("id", $rideRequest->id)
                ->first();

            (new UserServices())->createRequest(
                $Providers,
                $rideRequest,
                "TRANSPORT"
            );
    ...
    {
        $range_array = [];
        $range_data = GeoFence::select("id", "ranges")
    ...
    public function cancelRide($request)
    {
        try {
            ...
            if ($rideRequest->status == "CANCELLED") {
                return [
                    "status" => 404,
                    "message" => trans("api.ride.already_cancelled"),
                ];
            }

            if (
                in_array($rideRequest->status, [
                    "SEARCHING",
                    "STARTED",
                    "ARRIVED",
                    "SCHEDULED",
                ...
                if ($request->cancelled_by == "PROVIDER") {
                    if ($this->settings->site->broadcast_request == 1) {
                        return [
                            "status" => 200,
                            "message" => trans("api.ride.request_rejected"),
                        ];
                    } else {
                        (new ProviderServices())->assignNextProvider(
                            $rideRequest->id,
                            $rideRequest->admin_service
                        );
                        return [
                            "status" => 200,
                            "data" => $rideRequest->with("user")->get(),
                        ];
                    }
                } else {
                ...
                return [
                    "status" => 200,
                    "message" => trans("api.ride.ride_cancelled"),
                ];
            } else {
                return [
                    "status" => 403,
                    "message" => trans("api.ride.already_onride"),
                ];
            }
    ...
    public function extendTrip(Request $request)
    {
        try {
            ...
            $details =
                "https://maps.googleapis.com/maps/api/directions/json?origin=" .
                $rideRequest->s_latitude .
                "," .
                $rideRequest->s_longitude .
                "&destination=" .
                $request->latitude .
                "," .
                $request->longitude .
            ...
            $requestData = [
                ...
                "id" => $rideRequest->id,
                "city" =>
                    $this->settings->demo_mode == 0 ? $rideRequest->city_id : 0,
    ...
    public function updateRide(Request $request)
    {
        try {
            ...
            if (
                $request->status == "DROPPED" &&
                $rideRequest->payment_mode != "CASH"
            ) {
                $rideRequest->status = "COMPLETED";
                $rideRequest->paid = 0;

                (new SendPushNotification())->Complete(
                    $rideRequest,
                    "transport"
                );
            } elseif (
                ...
                $RequestPayment = RideRequestPayment::where(
                    "ride_request_id",
                    $request->id
                )->first();
                ...
                if ($request->status == "ARRIVED") {
                    (new SendPushNotification())->Arrived(
                        $rideRequest,
                        "transport"
                    );
                }
                ...
                if ($this->settings->transport->ride_otp == 1) {
                    if (
                        ...
                        if ($request->otp == $rideRequest->otp) {
                            ...
                            (new SendPushNotification())->Pickedup(
                                $rideRequest,
                                "transport"
                            );
                        } else {
            ...
            if ($request->status == "DROPPED") {
                ...
                $chat = Chat::where("admin_service", "TRANSPORT")
                    ->where("request_id", $rideRequest->id)
                    ->where(
                        "company_id",
                ...
                if (
                    $request->has("location_points") &&
                    $rideRequest->location_points != null
                ) {
                    ...
                    $rideRequest->route_key = $route_key;
                    $rideRequest->location_points = json_encode(
                        $request->location_points
                    );
                }
                ...
                $rideRequest->invoice = $this->invoice(
                    $request->id,
                    $request->toll_price != null ? $request->toll_price : 0
                );

                if ($rideRequest->invoice) {
                    (new SendPushNotification())->Dropped(
                        $rideRequest,
                        "transport"
                    );
                }
    ...
    {
        try {
            $rideRequest = RideRequest::with("provider")->findOrFail(
                $request_id
            );
            ...
            if (!empty($pricedata)) {
                ...
                $tax_percentage = isset($RideCityPrice->tax)
                    ? $RideCityPrice->tax
                    : 0;
                $commission_percentage = isset($RideCityPrice->commission)
                    ? $RideCityPrice->commission
                    : 0;
                $waiting_percentage = isset($RideCityPrice->waiting_commission)
                    ? $RideCityPrice->waiting_commission
                    : 0;
                $peak_percentage = isset($RideCityPrice->peak_commission)
                    ? $RideCityPrice->peak_commission
                    : 0;
            }
            ...
            if (!empty($rideRequest->admin_id)) {
                $Fleet = Admin::where("id", $rideRequest->admin_id)
                    ->where("type", "FLEET")
                    ->where(
                        "company_id",
                        Auth::guard("provider")->user()->company_id
                ...
                if (!empty($Fleet)) {
                    if (!empty($Commision)) {
                        ...
                        $fleet_per = $RideCityPrice->fleet_commission
                            ? $RideCityPrice->fleet_commission
                            : 0;
                    }
            ...
            if ($RideCityPrice->waiting_min_charge > 0) {
                $total_waiting = round(
                    $this->total_waiting($rideRequest->id) / 60
                );
                if ($total_waiting > 0) {
                    if ($total_waiting > $RideCityPrice->waiting_free_mins) {
                        $total_waiting_time =
                            $total_waiting - $RideCityPrice->waiting_free_mins;
                        $total_waiting_amount =
                            $total_waiting_time *
                            $RideCityPrice->waiting_min_charge;
                        $waiting_comm_amount =
                            ($waiting_percentage / 100) * $total_waiting_amount;
                    }
                }
            }
            ...
            $start_time_check = PeakHour::where("start_time", "<=", $start_time)
                ->where("end_time", ">=", $start_time)
                ->where("timezone", $rideRequest->timezone)
                ->where(
                    "company_id",
                    Auth::guard("provider")->user()->company_id
                )
            ...
            if ($start_time_check) {
                $RideCityPriceList = RideCityPrice::where(
                    "geofence_id",
                    $rideRequest->geofence_id
                )
                    ->where(
                        "ride_delivery_vehicle_id",
                        $rideRequest->ride_delivery_id
                    )
                    ->where(
                        "company_id",
                        Auth::guard("provider")->user()->company_id
                ...
                $Peakcharges = RidePeakPrice::where(
                    "ride_city_price_id",
                    $RideCityPriceList->id
                )
                    ->where("ride_delivery_id", $rideRequest->ride_delivery_id)
            ...
            $payable_amount +=
                $peakamount + $total_waiting_amount + $toll_price;

            $ProviderPay =
                $ProviderPay +
                ($peakamount + $total_waiting_amount) +
                $toll_price;
            ...
            if ($rideRequest->use_wallet == 1 && $payable_amount > 0) {
                ...
                if ($Wallet != 0) {
                    ...
                    (new SendPushNotification())->ChargedWalletMoney(
                        $rideRequest->user_id,
                        Helper::currencyFormat($wallet_det, $currencySymbol),
                        "transport"
                    );
                ...
                if ($rideRequest->payment_mode == "CASH") {
                    $Payment->round_of =
                        round($payable_amount) - abs($payable_amount);
                    $Payment->total = $Total;
    ...
    {
        $waiting = RideRequestWaitingTime::where("ride_request_id", $id)
            ->whereNotNull("ended_at")
            ->sum("waiting_mins");

        $uncounted_waiting = RideRequestWaitingTime::where(
            "ride_request_id",
            $id
        )
            ->whereNull("ended_at")
        ...
        if ($uncounted_waiting != null) {
            $waiting += Carbon::parse(
                $uncounted_waiting->started_at
    ...
    public function calculateFare($request, $cflag = 0)
    {
        try {
            ...
            if (!empty($location["errors"])) {
                ...
                $surge_trigger = isset($transportConfig->surge_trigger)
                    ? $transportConfig->surge_trigger
                    : 0;
                ...
                if ($cflag != 0) {
                    ...
                    $start_time_check = PeakHour::whereRaw(
                        "IF( start_time > end_time,  CONCAT( subdate(CURDATE(), 1), ' ', start_time ), CONCAT( CURDATE(), ' ', start_time ) ) <= '$start_time'"
                    )
                        ->whereRaw(
                            "CONCAT( CURDATE(), ' ', end_time ) >= '$start_time'"
                        )
    ...
    public function applyNumberFormat($total)
    {
        return round(
            $total,
            Helper::setting()->site->round_decimal != ""
                ? Helper::setting()->site->round_decimal
                : 2
        );
    }
