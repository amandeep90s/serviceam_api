...
class ProviderServices
{
    ...
    public function checkRequest($request)
    {
        try {
            ...
            $IncomingRequests = RequestFilter::with([
                "request.user",
                "request.service",
                "request.service",
                "request",
            ...
            if (!empty($IncomingRequests)) {
                for ($i = 0; $i < count($IncomingRequests); $i++) {
                    ...
                    elseif ($admin_service == "SERVICE") {
                        ...
                        $serviceRequestid =
                            $IncomingRequests[$i]->request->request_id;
                        $serveRequest = ServiceRequest::select(
                            "id",
                            "service_id"
                        )
                        ...
                        if ($serveRequest != null) {
                            $IncomingRequests[$i]->request->service_details = Service::where(
                                "id",
                                $serveRequest->service_id
                            )->first();
                        } else {
                    ...
                    elseif ($admin_service == "ORDER") {
                        ...
                        $IncomingRequests[$i]->request->store_orders = $orderRequest = StoreOrder::select(
                            "id",
                            "store_id",
                            "delivery_address",
                            "pickup_address"
            ...
            $Reason = Reason::select("id", "reason")
                ->where("type", "PROVIDER")
                ->get();

            $referral = (new ReferralResource())->get_referral(
                "provider",
                $this->user->id
            )[0];
    ...
    public function assignProvider($config, $IncomingRequest)
    {
        if ($config != "" && $config->manual_request == 0) {
            ...
            $IncomingRequest->request->time_left_to_respond =
                $Timeout -
                (time() -
                    strtotime($IncomingRequest->request->request->assigned_at));

            if (
                $IncomingRequest->request->status == "SEARCHING" &&
                $IncomingRequest->request->time_left_to_respond < 0
            ) {
                if ($config->broadcast_request == 1) {
                    $this->assignDestroy(
                        $IncomingRequest->request->request->id,
                        $IncomingRequest->request->admin_service
                    );
                } else {
                    $this->assignNextProvider(
                        $IncomingRequest->request->request->id,
                        $IncomingRequest->request->admin_service
                    );
                }
    ...
    public function assignDestroy($id, $admin_service)
    {
        if ($admin_service == "ORDER") {
            $UserRequest = UserRequest::where("admin_service", $admin_service)
                ->where("request_id", $id)
                ->first();
            if ($UserRequest != "") {
                $request_filter = RequestFilter::where(
                    "request_id",
                    $UserRequest->id
                )
        ...
        $userRequestLast = UserRequest::where("status", "SEARCHING")
            ->where("admin_service", $admin_service)
            ->orderby("id")
            ->first();
        ...
        if ($userRequestLast != null) {
            if ($admin_service == "TRANSPORT") {
                ...
                $callback = function ($q) use ($newRequest) {
                    ...
                    $q->where(
                        "ride_delivery_id",
                        $newRequest->ride_delivery_id
                    );
                };

                $withCallback = [
                    "service" => $callback,
                    "service.ride_vehicle",
                ];
                ...
                if (!empty($Providers)) {
                    foreach ($Providers as $key => $Provider) {
                        ...
                        if (
                            $this->settings->transport->broadcast_request == 1
                        ) {
                            (new SendPushNotification())->IncomingRequest(
                                $Provider->id,
                                "transport_incoming_request",
                                "Transport Incoming Request"
                            );
                        }
                    }
    ...
    public function cancel($id, $admin_service)
    {
        if ($admin_service == "TRANSPORT") {
                ...
                if ($admin_service == "ORDER") {
                    ...
                    if (count($other_request_filter) <= 0) {
                        ...
                        $storedisputedata = StoreOrderDispute::where(
                            ...
                            ->where("dispute_name", "Provider Not Available")
                            ->where("status", "open")
                            ->get();
                        if (count($storedisputedata) == 0) {
                            ...
                            $storedispute->dispute_name =
                                "Provider Not Available";
                            $storedispute->dispute_type_comments =
                                "Provider Not Available";
            ...
            try {
                $newRequest = \App\Models\Service\ServiceRequest::findOrFail(
                    $id
                );
                $newRequest->status = "CANCELLED";
                $newRequest->save();
    ...
    public function assignNextProvider($request_id, $admin_service)
    {
        ...
        try {
            $nextRequestFilter = RequestFilter::where(
                "admin_service",
                $admin_service
            )
                ->where("request_id", $UserRequest->id)
            ...
            if ($nextRequestFilter != null) {
                ...
                (new SendPushNotification())->IncomingRequest(
                    $nextRequestFilter->provider_id,
                    strtolower($admin_service)
                );
            } else {
                $UserRequest->delete();
            ...
            (new SendPushNotification())->ProviderNotAvailable(
                $UserRequest->user_id,
                strtolower($admin_service)
            );
        }
    ...
    public function acceptRequest(Request $request)
    {
        try {
                ...
                if ($request->admin_service == "SERVICE") {
                    ...
                    if ($newRequest->status != "SEARCHING") {
                        return [
                            "status" => 422,
                            "message" => trans(
                                "api.service.request_inprogress"
                            ),
                        ];
                    }
                }
            ...
            try {
                if ($request->admin_service == "ORDER") {
                    ...
                    try {
                        ...
                        return [
                            "status" => 500,
                            "message" => trans("api.order.order_not_found"),
                        ];
                    }
                    if ($newRequest->status != "SEARCHING") {
                        return [
                            "status" => 422,
                            "message" => trans("api.order.request_inprogress"),
                        ];
                    }
                }
            } catch (\Throwable $e) {
            ...
            $user_request = UserRequest::where("request_id", $newRequest->id)
            ...
            if ($newRequest->schedule_at != "") {
                $beforeschedule_time = strtotime(
                    $newRequest->schedule_at . "- 1 hour"
                );
                $afterschedule_time = strtotime(
                    $newRequest->schedule_at . "+ 1 hour"
                );

                try {
                    if ($request->admin_service == "TRANSPORT") {
                        $CheckScheduling = \App\Models\Transport\RideRequest::where(
                            "status",
                            "SCHEDULED"
                        )
                        ...
                        if ($CheckScheduling > 0) {
                            return [
                                "status" => 403,
                                "message" => trans(
                                    "api.ride.request_already_scheduled"
                                ),
                            ];
                        }
                    }
                ...
                try {
                    if ($request->admin_service == "ORDER") {
                        $CheckScheduling = \App\Models\Order\StoreOrder::where(
                            "status",
                            "SCHEDULED"
                        )
                            ->where("provider_id", $this->user->id)
                        ...
                        if ($CheckScheduling > 0) {
                            return [
                                "status" => 403,
                                "message" => trans(
                                    "api.order.request_already_scheduled"
                                ),
                            ];
                        }
                    }
                ...
                try {
                    if ($request->admin_service == "SERVICE") {
                        $CheckScheduling = \App\Models\Service\ServiceRequest::where(
                            "status",
                            "SCHEDULED"
                        )
                            ->where("provider_id", $this->user->id)
                        ...
                        if ($CheckScheduling > 0) {
                            return [
                                "status" => 403,
                                "message" => trans(
                                    "api.service.request_already_scheduled"
                                ),
                            ];
                        }
                    }
            ...
            $Filters = RequestFilter::select("id")
                ->where("admin_service", $user_request->admin_service)
                ->where("request_id", $user_request->id)
                ->where("provider_id", "!=", $this->user->id)
                ->get();

            $user_request_last = UserRequest::where("status", "SEARCHING")
                ->where("admin_service", $request->admin_service)
                ->orderby("id", "desc")
                ->first();

            if ($user_request_last != null) {
                DB::table("request_filters")
                    ->whereIn("id", $Filters)
                    ->update(["request_id" => $user_request_last->id]);
            ...
            if ($request->admin_service == "TRANSPORT") {
                (new SendPushNotification())->RideAccepted(
                    $newRequest,
                    strtolower($request->admin_service),
                    trans("api.ride.request_accepted")
                );
                return [
                    "status" => 200,
                    "message" => trans("api.ride.request_accepted"),
                    "data" => $newRequest,
                ];
            } elseif ($request->admin_service == "ORDER") {
                (new SendPushNotification())->RideAccepted(
                    $newRequest,
                    strtolower($request->admin_service),
                    trans("api.order.request_accepted")
                );
                return [
                    "status" => 200,
                    "message" => trans("api.order.request_accepted"),
                    "data" => $newRequest,
                ];
                ...
                (new SendPushNotification())->RideAccepted(
                    $newRequest,
                    strtolower($request->admin_service),
                    trans("api.service.request_accepted")
                );
                return [
                    "status" => 200,
                    "message" => trans("api.service.request_accepted"),
                    "data" => $newRequest,
                ];
            }
        } catch (Exception $e) {
    ...
    public function cancelRequest(Request $request)
    {
        ...
        if ($UserRequest == null) {
            if ($request->admin_service == "TRANSPORT") {
                ...
                return Helper::getResponse([
                    "message" => trans("api.order.request_rejected"),
                ]);
            } elseif ($request->admin_service == "SERVICE") {
                return Helper::getResponse([
        ...
        $request_filter = RequestFilter::select("id")
            ->where("request_id", $UserRequest->id)
            ->where("admin_service", $request->admin_service)
            ->where("provider_id", $this->user->id)
            ->first();
    ...
    {
        try {
            $ratingRequest = Rating::where("request_id", $newRequest->id)
                ->where("admin_service", $newRequest->admin_service)
                ->first();
            ...
            $average = Rating::where("user_id", $newRequest->user_id)->avg(
                "provider_rating"
            );
    ...
    public function providerHistory(
        ...
        try {
            ...
            if ($type == "new") {
                $historyStatus = ["SEARCHING", "SCHEDULED"];
            } elseif ($type == "inride") {
                $historyStatus = [
                    "ACCEPTED",
                    "STARTED",
            ...
            if (!empty($data)) {
                ...
                foreach ($data as $key => $value) {
                    ...
                    $data[$key]->static_map =
                        ...
                        "," .
                        $value->d_longitude .
                        "&path=color:0x000000|weight:3|enc:" .
                        $value->route_key .
                        "&key=" .
