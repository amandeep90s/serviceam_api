...
class UserServices
{
    ...
    public function createRequest($Providers, $newRequest, $type)
    {
        ...
        if ($newRequest->status != "SCHEDULED") {
            if ($this->settings->transport->manual_request == 0) {
                ...
                foreach ($Providers as $key => $Provider) {
                    ...
                    if ($existingRequest == null) {
                        ...
                        if (
                            $this->settings->transport->broadcast_request ==
                            0 &&
            ...
            (new SendPushNotification())->ProviderCancelRide(
                $newRequest,
                "transport"
            );
    ...
    public function cancelRequest($newRequest)
    {
        try {
            ...
            RequestFilter::where("admin_service", $newRequest->admin_service)
            ...
            if ($newRequest->cancelled_by == "PROVIDER") {
                ...
                // Send Push Notification to User
                (new SendPushNotification())->UserCancelRide(
                    $newRequest,
                    "transport"
                );
            }
    ...
    public function updatePaymentMode(
        ...
        try {
            ...
            (new SendPushNotification())->updateProviderStatus(
                ...
                trans("api.ride.payment_updated"),
                "provider"
            );
        ...
        try {
            ...
            if (@$ratingRequest == null) {
                Rating::create([
                    ...
                    "store_rating" => $request->has("shoprating")
                        ? $request->shoprating
                        : 0,
            ...
            if ($newRequest->admin_service != "ORDER") {
                ...
                $newRequest->order_type == "DELIVERY" &&
                $newRequest->admin_service == "ORDER"
            ) {
                $provider->rating = $average;
                $provider->save();
            }
    ...
    public function availableProviders(
        ...
        $distance = isset($this->settings->transport->provider_search_radius)
        ...
        $data->select(
            ...
            "longitude",
            DB::Raw(
                "(3959 * acos( cos( radians('$request->latitude') ) * cos( radians(latitude) ) * cos( radians(longitude) - radians('$request->longitude') ) + sin( radians('$request->latitude') ) * sin( radians(latitude) ) ) ) AS distance"
            )
        );
        ...
        $data->whereRaw(
            "(3959 * acos( cos( radians('$request->latitude') ) * cos( radians(latitude) ) * cos( radians(longitude) - radians('$request->longitude') ) + sin( radians('$request->latitude') ) * sin( radians(latitude) ) ) ) <= $distance"
        );
    ...
    public function payment(Request $request, $UserRequest, $payment)
    {
        try {
            ...
            $paymentConfig = json_decode(
                json_encode($this->settings->payment),
                true
            );
            ...
            if ($UserRequest->admin_service == "ORDER") {
                $publishUrl = "checkOrderRequest";
            }
            if ($UserRequest->admin_service == "SERVICE") {
            ...
            $random =
                $this->settings->transport->booking_prefix .
