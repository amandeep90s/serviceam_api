...
class Order
{
    ...
    public function cancelOrder($request)
    {
        try {
            ...
            if ($orderRequest->status == "CANCELLED") {
                return [
                    "status" => 404,
                    "message" => trans("api.order.ride_cancelled"),
                ];
            }

            if (
                in_array($orderRequest->status, ["ORDERED", "STORECANCELLED"])
            ) {
        ...
        try {
            ...
            if ($request->has("delivery_date")) {
                $delivery_date = Carbon::createFromFormat(
                    "Y-m-d H:i:s",
                    Carbon::parse($request->delivery_date)->format(
                        "Y-m-d H:i:s"
                    ),
                    $timezone
                )->setTimezone("UTC");
            ...
            $storeorderstatus->company_id = Auth::guard(
                "shop"
            )->user()->company_id;
            $storeorderstatus->save();
            $requestData = [
                "type" => "ORDER",
                "room" => "room_" . Auth::guard("shop")->user()->company_id,
                "id" => $request->store_order_id,
                "shop" => Auth::guard("shop")->user()->id,
    ...
    {
        StoreOrder::where("id", $request->id)->update([
            "status" => "STORECANCELLED",
        ]);
    ...
    public function callTransaction($store_order_id)
    {
        ...
        if ($StoreOrder->paid == 1) {
            ...
            $transation["transaction_alias"] =
                $StoreOrder->store_order_invoice_id;

            $paymentsStore = StoreOrderInvoice::where(
                "store_order_id",
                $store_order_id
            )->first();
            ...
            $credit_amount =
                $paymentsStore->total_amount -
                $paymentsStore->commision_amount -
                $paymentsStore->delivery_amount;
    ...
    protected function createAdminWallet($request)
    {
        ...
        if (empty($admin_data->close_balance)) {
            ...
            $adminwallet->close_balance =
                $admin_data->close_balance + $request["amount"];
        }
    ...
    protected function createShopWallet($request)
    {
        ...
        if (empty($store->wallet_balance)) {
            ...
            $storeWallet->close_balance =
                $store->wallet_balance + $request["amount"];
        }
