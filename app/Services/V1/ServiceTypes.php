    ...
    public function calculateFare($request, $cflag = 0)
    {
        try {
            ...
            if (!empty($location["errors"])) {
                ...
                $tax_percentage = isset($price_response["ride_city_price"]->tax)
                    ? $price_response["ride_city_price"]->tax
                    : 0;
                $commission_percentage = isset(
                    $price_response["ride_city_price"]->commission
                )
                    ? $price_response["ride_city_price"]->commission
                    : 0;
