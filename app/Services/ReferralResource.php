...
class ReferralResource
{
    ...
    )
    {
        if ($referrer == "user") {
            ...
            $referral = User::where("referral_unique_id", $referral_code)
                ->where("company_id", $referral_data->company_id)
                ->first();
        } elseif ($referrer == "provider") {
            $type = 2;
            $referral = Provider::where("referral_unique_id", $referral_code)
                ->where("company_id", $referral_data->company_id)
                ->first();
        }
        ...
        if (!empty($referral)) {
            ...
            if ($siteConfig->referral_amount > 0) {
                $ReferralHistory = ReferralHistory::select(
                ...
                if (
                    $siteConfig->referral_count > 0 &&
                    $siteConfig->referral_count == $Referralcount
                ) {
                    ...
                    (new Transactions())->referralCreditDebit(
                        $transaction,
                        $type
                    );

                    //update histroy table process status to complete
                    ReferralHistory::where("referrer_id", $referral->id)
                        ->where("type", $type)
                        ->where("status", "P")
                        ->where("company_id", $referral->company_id)
                        ->update(["status" => "C"]);
    ...
    {
        $ReferralHistory = ReferralEarning::where("referrer_id", $user_id)
            ->where("type", $type)
            ->get([
