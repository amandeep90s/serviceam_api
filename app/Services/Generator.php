...
class Generator
{
    public static function generateDocs($version = null): void
    {
        ...
        if (!File::exists($docDir) || is_writable($docDir)) {
            ...
            $excludeDirs = array_diff(
                array_filter(glob("app/Http/Controllers/V*"), "is_dir"),
                array_filter(
                    glob("app/Http/Controllers/V" . $version),
                    "is_dir"
                )
            );
            ...
            if (
                version_compare(
                    config("swagger-lume.swagger_version"),
                    "3.0",
                    ">="
                )
            ) {
