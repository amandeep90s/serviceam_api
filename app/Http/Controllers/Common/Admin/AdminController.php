...
class AdminController extends Controller
{
            ...
            if ($request->has("payment_name")) {
                ...
                foreach ($request->payment_name as $key => $value) {
                    ...
                    foreach ($payment_key_name as $k => $credential_name) {
                        if (
                            preg_match("#^" . $key . "_#", $credential_name) ===
                            1
                        ) {
                            $credentials[] = [
                                "name" => str_replace(
    ...
    {
        try {
            $data["currency"] = CompanyCountry::where("country_id", $id)
                ->select("currency")
                ->first();
            ...
            $data["provider_debit"] = Provider::select(
                DB::raw(
                    "SUM(CASE WHEN wallet_balance<0 THEN wallet_balance ELSE 0 END) as total_debit"
                )
            )
                ->where("country_id", $id)
                ->where("company_id", Auth::user()->company_id)
            ...
            $data["provider_credit"] = Provider::select(
                DB::raw(
                    "SUM(CASE WHEN wallet_balance>=0 THEN wallet_balance ELSE 0 END) as total_credit"
                )
            )
                ->where("country_id", $id)
                ->where("company_id", Auth::user()->company_id)
