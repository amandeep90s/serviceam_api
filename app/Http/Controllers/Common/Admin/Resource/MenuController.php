...
class MenuController extends Controller
{
    ...
    {
        $datum = Menu::with("adminservice")->where(
            "company_id",
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        if ($request->has("is_featured") && $request->is_featured == 1) {
            $this->validate($request, ["featured_image" => "required"]);
        }
    ...
    public function show($id): JsonResponse
    {
        try {
            ...
            if ($menu->admin_service == "SERVICE") {
                $menu["service_list"] = ServiceCategory::where(
                    "company_id",
                    Auth::user()->company_id
                )->get();
            } elseif ($menu->admin_service == "TRANSPORT") {
                $menu["service_list"] = RideType::where(
                    "company_id",
                    Auth::user()->company_id
                )
                    ->where("status", 1)
    ...
    public function menucity(Request $request, $id): JsonResponse
    {
        $city_details = $request->city_id;
        $country_id = isset($request->country_id) ? $request->country_id : "";
        try {
            if ($country_id == "ALL" || $country_id == "") {
                MenuCity::where("menu_id", $id)->delete();
            } else {
                MenuCity::where("menu_id", $id)
            ...
            if ($city_details != "") {
                ...
                return Helper::getResponse([
                    "status" => 404,
                    "message" => "You have to check atleast one City.",
                ]);
            }
        } catch (\Throwable $e) {
            return Helper::getResponse([
    ...
    //Ride type value
    public function ride_type(): JsonResponse
    {
        $vehicle_type_list = RideType::where(
            "company_id",
            Auth::user()->company_id
        )
            ->where("status", 1)
    ...
    //Service Type
    public function service_type(): JsonResponse
    {
    ...
    public function getCountryCity($serviceId, $countryId): JsonResponse
    {
        $company_cities = CompanyCity::with("city")
    ...
    public function getmenucity($id): array
    {
        $city_details = [];
        $menu_city = MenuCity::select("city_id")
            ->where("menu_id", $id)
            ->get();
    ...
    public function order_type(): JsonResponse
    {
        $service_list = StoreType::where("company_id", Auth::user()->company_id)
            ->where("status", 1)
            ->get();
        return Helper::getResponse(["data" => $service_list]);
    }
