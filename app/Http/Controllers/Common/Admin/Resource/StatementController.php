...
class StatementController extends Controller
{
    ...
    public function statement_provider(Request $request)
    {
        try {
            $servicenames = [];
            $adminservices = AdminService::where(
                "company_id",
            ...
            $datum = Provider::select("*", "created_at as joined")->where(
                "company_id",
                Auth::user()->company_id
            );
            ...
            foreach ($Providers as $index => $Provider) {
                ...
                $servicePayment = 0;
                $orderPayment = 0;
                if (in_array("TRANSPORT", $servicenames)) {
                    $Rides = RideRequest::where(
                        "provider_id",
                        $Provider->id
                    )
                    ...
                    $ridePaymentQ = RideRequestPayment::whereIn(
                        "ride_request_id",
                        $Rides
                    )
                        ->select(
                            \DB::raw("SUM(ROUND(provider_pay)) as overall")
                        )
                        ->first();
                ...
                if (in_array("SERVICE", $servicenames)) {
                    ...
                    $servicepaymentQ = \App\Models\Service\ServiceRequestPayment::whereIn(
                        "service_request_id",
                        $services
                    )
                        ->select(
                            \DB::raw("SUM(ROUND(total)) as serviceoverall")
                        )
                        ->first();
                ...
                if (in_array("ORDER", $servicenames)) {
                    ...
                    $orderpaymentQ = \App\Models\Order\StoreOrderInvoice::whereIn(
                        "store_order_id",
                        $Orders
                    )
                        ->select(
                ...
                $Providers[$index]->payment =
                    $ridePayment + $servicePayment + $orderPayment;
            }
            ...
            foreach ($Users as $index => $User) {
                ...
                if (in_array("TRANSPORT", $servicenames) == true) {
                    ...
                    $ridePaymentQ = RideRequestPayment::whereIn(
                        ...
                        ->select(\DB::raw("SUM(ROUND(total)) as overall"))
                        ->first();
                    if ($ridePaymentQ != null) {
                        $ridePayment = $ridePaymentQ->overall;
                    }
    ...
    {
        try {
            $datum = AdminWallet::select("*", "created_at as dated")->where(
                "company_id",
                Auth::user()->company_id
            );
            if ($request->has("search_text") && $request->search_text != null) {
    ...
    {
        try {
            $fleets = \App\Models\Common\Admin::where(
                "company_id",
                Auth::user()->company_id
            )->where("type", "FLEET");
            ...
            $providers = \App\Models\Common\Provider::where(
                "company_id",
                Auth::user()->company_id
            );
            ...
            if ($request->service_type == "TRANSPORT") {
                $ride_type = \App\Models\Transport\RideType::where(
                    "company_id",
                    Auth::user()->company_id
                )
                    ->select("id", "ride_name")
            ...
            if ($request->service_type == "SERVICE") {
                $ride_type = \App\Models\Service\ServiceCategory::where(
                    "company_id",
                    Auth::user()->company_id
                )
            ...
            if ($request->service_type == "ORDER") {
                $ride_type = \App\Models\Order\StoreType::where(
                    "company_id",
                    Auth::user()->company_id
                )
                    ->select("id", "name")
