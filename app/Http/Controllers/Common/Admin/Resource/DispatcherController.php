...
class DispatcherController extends Controller
{
    ...
    public function trips(Request $request)
    {
        ...
        $trips = UserRequest::with("user", "provider", "service")->orderBy(
            "id",
            "desc"
        );
    ...
    public function providers(Request $request)
    {
        if ($request->has("latitude") && $request->has("longitude")) {
            ...
            if ($request->has("provider_service_id")) {
                $ActiveProviders = ProviderService::where(
                    "company_id",
                    Auth::user()->company_id
                )
                    ->where("ride_delivery_id", $request->provider_service_id)
                    ->where("admin_service", "TRANSPORT")
                    ->get()
                    ->pluck("provider_id");
                $transportConfig = $settings->transport;
                $distance = isset($transportConfig->provider_search_radius)
                    ? $transportConfig->provider_search_radius
                    : 10;
            ...
            if ($request->has("store_type_id")) {
                $ActiveProviders = ProviderService::where(
                    ...
                    ->where("category_id", $request->store_type_id)
                    ->where("admin_service", "ORDER")
                    ->get()
                    ->pluck("provider_id");
                $orderConfig = $settings->order;
                $distance = isset($orderConfig->store_search_radius)
                    ? $orderConfig->store_search_radius
                    : 100;
                $admin_service = "ORDER";
            }
                ...
                ->with([
                    "service" => function ($query) use ($admin_service, $request) {
                        if ($admin_service == "ORDER") {
                            ...
                            $query->where(
                                "category_id",
                                $request->store_type_id
                            );
                        } elseif ($admin_service == "TRANSPORT") {
    ...
    public function assign(Request $request)
    {
        ...
        if ($request->admin_service == "TRANSPORT") {
            ...
            $requestData = [
                ...
                "id" => $newRequest->id,
                "city" => $setting->demo_mode == 0 ? $newRequest->city_id : 0,
        ...
        Provider::where("id", $newRequest->provider_id)->update([
            "is_assigned" => "1",
        ]);
        (new SendPushNotification())->IncomingRequest(
            $newRequest->provider_id,
            $type
        );
        if ($request->admin_service == "TRANSPORT") {
        ...
        return Helper::getResponse([
            "message" => trans("admin.dispatcher_msgs.request_assigned"),
        ]);
    }
    ...
    public function create_ride(Request $request)
    {
        ...
        if ($request->schedule_date != "" && $request->schedule_time != "") {
            try {
                $schedule_time = Carbon::createFromFormat(
                    "Y-m-d H:i:s",
                    Carbon::parse(
                ...
                $CheckScheduling = \App\Models\Transport\RideRequest::where(
                    ...
                    ->where(
                        "schedule_at",
                        ">",
                        strtotime($schedule_time . " - 1 hour")
                    )
                    ->where(
                        "schedule_at",
                        "<",
                ...
                return Helper::getResponse([
                    "message" => trans("api.ride.request_scheduled"),
                    "error" => trans("api.ride.request_scheduled"),
                ]);
            } catch (Exception $e) {
                // Do Nothing
            }
        ...
        $distance = $transportConfig->provider_search_radius
            ? $transportConfig->provider_search_radius
            : 100;
        ...
        try {
            ...
            $newRequest->s_address = $request->s_address
                ? $request->s_address
                : "";
            $newRequest->d_address = $request->d_address
                ? $request->d_address
                : "";
            ...
            $newRequest->track_latitude = $request->d_latitude
                ? $request->d_latitude
                : $request->s_latitude;
            $newRequest->track_longitude = $request->d_longitude
                ? $request->d_longitude
                : $request->s_longitude;
            ...
            $newRequest->destination_log = json_encode([
                [
                    "latitude" => $newRequest->d_latitude,
                    "longitude" => $request->d_longitude,
                    "address" => $request->d_address,
                ],
            ]);
            $newRequest->distance = $request->distance ? $request->distance : 0;
            $newRequest->unit = isset($siteConfig->distance)
                ? $siteConfig->distance
                : "Kms";
            ...
            if (
                ...
                $newRequest->schedule_at = Carbon::createFromFormat(
                    "Y-m-d H:i:s",
                    Carbon::parse(
                        $request->schedule_date . " " . $request->schedule_time
                    )->format("Y-m-d H:i:s"),
            ...
            $newRequest = RideRequest::with("ride", "ride_type")
                ...
                if ($newRequest->status != "SCHEDULED") {
                    if ($transportConfig->manual_request == 0) {
                        ...
                        foreach ($Providers as $key => $Provider) {
                            ...
                            $existingRequest = RequestFilter::where(
                                "provider_id",
                                $Provider->id
                            )->first();
                            if ($existingRequest == null) {
                                ...
                                $Filter->admin_service =
                                    $newRequest->admin_service;
                                $Filter->request_id = $user_request->id;
                                $Filter->provider_id = $Provider->id;

                                if (
                                    $transportConfig->broadcast_request == 0 &&
    ...
    public function cancel_ride(Request $request)
    {
        ...
        try {
            ...
            if (
                ...
                if ($request->admin_service == "TRANSPORT") {
                    try {
                        $rideRequest = \App\Models\Transport\RideRequest::find(
                            $request->id
                        );
                    ...
                    try {
                        $serviceRequest = \App\Models\Service\ServiceRequest::find(
                            $request->id
                        );
                ...
                RequestFilter::where("admin_service", $request->admin_service)
    ...
    public function create_service(Request $request)
    {
        ...
        $timezone = $User->state_id
            ? State::find($User->state_id)->timezone
            : "";

        $currency = CompanyCountry::where("company_id", $company_id)
        ...
        try {
            ...
            if (
                ...
                trim($request->schedule_date) != "" &&
                trim($request->schedule_time) != ""
            ) {
    ...
    public function cancel_service(Request $request)
    {
        ...
        try {
            $newRequest = UserRequest::where("admin_service", "SERVICE")
                ->where("request_id", $request->id)
                ->first();
            ...
            if (
                ...
                RequestFilter::where("admin_service", "SERVICE")
                    ->where("request_id", $newRequest->id)
                    ->delete();
                UserRequest::where("id", $newRequest->id)->delete();
                $newRequest->delete();
    ...
    public function fare(Request $request)
    {
        ...
        try {
            if ($request->admin_service == "TRANSPORT") {
                $newRequest = \App\Models\Transport\RideRequest::find(
                    $request->id
                );
            }
        } catch (\Throwable $e) {
        ...
        try {
            ...
            $request->request->add([
                "service_type" => $request->provider_service,
            ]);
        ...
        if ($provider_service) {
            ...
            if (!empty($provider_service[0]->service)) {
                $serviceDetails = Service::with("serviceCategory")
                    ...
                    ->where("company_id", Auth::user()->company_id)
                    ->first();
                foreach ($provider_service as $service) {
                    ...
                    if ($service->service_city == null) {
                        ...
                        if (
                            ...
                            if (!empty($request->qty)) {
                                $provider->base_fare = Helper::decimalRoundOff(
                                    $service->service->base_fare * $request->qty
                                );
                            } else {
                                $provider->base_fare = Helper::decimalRoundOff(
