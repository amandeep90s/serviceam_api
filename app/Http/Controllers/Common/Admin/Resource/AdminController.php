...
class AdminController extends Controller
{
    ...
    public function heatmap(Request $request)
    {
        try {
            ...
            if ($type == "TRANSPORT") {
                try {
                    $services = \App\Models\Transport\RideRequest::whereIn(
                        "status",
                        ["SEARCHING"]
                    )
                        ->orderBy("id", "desc")
                ...
                try {
                    $services = \App\Models\Order\StoreOrder::whereIn(
                        "status",
                        ["SEARCHING"]
                    )
                        ->orderBy("id", "desc")
                ...
                try {
                    $services = \App\Models\Service\ServiceRequest::whereIn(
                        "status",
                        ["SEARCHING"]
                    )
                        ->orderBy("id", "desc")
            ...
            foreach ($services as $service) {
                $data[] = [
                    "lat" => $service->s_latitude,
                    "lng" => $service->s_longitude,
                ];
            }
            return Helper::getResponse(["data" => $data]);
        } catch (\Throwable $e) {
            ...
            if ($type == "TRANSPORT") {
                if (
                    ...
                    $providers = Provider::with([
                        "request",
                        "current_vehicle.ride_vehicle",
                        "current_vehicle.ride_vehicle.ride_type",
                ...
                if (
                    ...
                    $providers = Provider::with([
                        "request",
                        "current_store_detail",
                        "current_store_detail.storetype",
                    ])
                        ->whereHas("request", function ($query) use ($status) {
                            $query
                                ->where("status", $status)
